...
public class HXImage: UIImage {
    //图片要放在外面，别放在Assets.xcassets里面，不然访问不了
  public class func imageNamePDF(name:String) -> UIImage{
        var fileName = name
        ...
        let pdfImage:UIImage = UIImage(cgImage: image!, scale: screenScale, orientation: UIImageOrientation.up)
        return pdfImage
    }
