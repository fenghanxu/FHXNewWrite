...
public class Time: NSObject {
  ...
  public enum CurrentTime:String {
    /**
     yyy-MM-dd
     */
  ...
  /**
   获取当前时间
   */
  public class func getCurrentTime(CurrentTime type:CurrentTime) -> String {
  ...
  /**
   计算现在到N天后的日期
   */
 public class func getDistanceTimeBehind(day:Int, AfterTime type:AfterTime) -> String{
    let timeFormatter = DateFormatter()
    timeFormatter.dateFormat = type.rawValue
  ...
  /**
   计算现在到N天前的日期
   */
 public class func getDistanceTimeFront(day:Int, AfterTime type:AfterTime) -> String{
    let timeFormatter = DateFormatter()
    timeFormatter.dateFormat = type.rawValue
    let lastDay = Date(timeInterval: TimeInterval(-(24*day*60*60)), since: Date())
    let yerDay = timeFormatter.string(from: lastDay) as String
  ...
  /**
   把时间转换成时间戳
   */
 public class func stringToTimeStamp(stringTime:String, CurrentTime type:CurrentTime)->String {
  ...
  /**
   把时间戳转换成时间
   */
 public class func timeStampToString(timeStamp:String, CurrentTime type:CurrentTime)->String {
  ...
  /**
   获取今天运行了几秒
   */
 public class func todayRuningHowmuchTime() ->Int {
    //获取当前时间精确到秒
  ...
  /**
   ...
   HH:mm:ss
   */
 public class func NTimeRunningSecond(hourMinSecond:String) ->Int {
    let currentDayBeginTime = getCurrentTime(CurrentTime: .time_0)
  ...
  /**
   获取某一个具体时间之前N天的时间
   */
 public class func getStartTimeIntervalDayBetween(startTime:String, intputType inType:Time.CurrentTime, intervalDay:Int, outputType outType:AfterTime) -> String{
  ...
  /**
   获取某一个具体时间之后N天的时间
   */
 public class func getStartTimeIntervalDayAfter(startTime:String, intputType inType:Time.CurrentTime, intervalDay:Int, outputType outType:AfterTime) -> String{
    //UTC国际标准  中国时间相差8小时28800
    let stamp = Time.stringToTimeStamp(stringTime: startTime, CurrentTime: inType)
    let beijingTime = String(Int(stamp)! + 28800)
    let dateLast = Date(timeIntervalSince1970: TimeInterval(beijingTime)!)
